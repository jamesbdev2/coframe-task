// Session URL:
// https://mark-1.coframe.com/?session_id=96bd0393-da08-47fb-b221-3af278188e36
...
(()=>{var g=Symbol.for("JSX.Fragment");function e(t,s,...f){if(t==null){let n=b(),a=s&&Object.keys(s).length>0,o=f&&f.length>0,l;a?l=`<${t}
  ${Object.entries(s).map(([d,i])=>`${d}=${JSON.stringify(i)}`).join(`
  `)}${o?">":""}`:l=`<${t}${o?">":""}`;let r;throw o?r=`
...
`)}if(t===g){let n=document.createDocumentFragment();return u(n,f),n}if(typeof t=="function")return t({...s||{},children:h(f)});let c=typeof t=="string"?document.createElement(t):document.createElement(String(t));return s&&v(c,s),u(c,f),c}function b(){return new Error().stack?.split(`
`)?.[3]?.trim().replace(/^at\s+/,"")}function v(t,s){for(let f in s){if(f==="key")continue;let c=s[f];if(f==="ref"){typeof c=="function"?c(t):c&&typeof c=="object"&&(c.current=t);continue}if(f==="class"||f==="className"){N(t,c);continue}if(f==="style"){w(t,c);continue}if(f==="dataset"&&c&&typeof c=="object"){for(let a in c)t.dataset[a]=String(c[a]);continue}if(f==="dangerouslySetInnerHTML"&&c&&typeof c=="object"){t.innerHTML=c.__html??"";continue}if(f.startsWith("on")&&typeof c=="function"){let a=f.slice(2).toLowerCase();t.addEventListener(a,c);continue}let n=f==="for"?"htmlFor":f;try{n in t&&typeof t[n]!="function"?t[n]=c:c!==!1&&c!=null&&t.setAttribute(f,String(c))}catch{c!==!1&&c!=null&&t.setAttribute(f,String(c))}}}function N(t,s){s&&(typeof s=="string"?t.className=s:Array.isArray(s)?t.className=s.filter(Boolean).join(" "):typeof s=="object"&&(t.className=Object.keys(s).filter(f=>s[f]).join(" ")))}function w(t,s){if(!s)return;if(typeof s=="string"){t.style.cssText=s;return}let f=t.style;for(let c in s){let n=s[c];n==null||n===!1||(c in f?f[c]=String(n):t.style.setProperty(c,String(n)))}}function u(t,s){let f=c=>{c==null||c===!1||(Array.isArray(c)?c.forEach(f):c instanceof Node?t.appendChild(c):t.appendChild(document.createTextNode(String(c))))};s.forEach(f)}function h(t){return t.length<=1?t[0]:t}var y=document.createElement("style");y.textContent=`
  /* Remove pseudo-elements from .hip-lp */
  .hip-lp::before,
  .hip-lp::after {
    display: none !important;
