// Session URL:
// https://mark-1.coframe.com/?session_id=6e51d077-0c86-4f82-8ed0-5d715a1ecda2
...
(()=>{var d=Symbol.for("JSX.Fragment");function e(t,s,...n){if(t==null){let a=y(),o=s&&Object.keys(s).length>0,r=n&&n.length>0,f;o?f=`<${t}
  ${Object.entries(s).map(([p,x])=>`${p}=${JSON.stringify(x)}`).join(`
  `)}${r?">":""}`:f=`<${t}${r?">":""}`;let i;throw r?i=`
...
`)}if(t===d){let a=document.createDocumentFragment();return l(a,n),a}if(typeof t=="function")return t({...s||{},children:h(n)});let c=typeof t=="string"?document.createElement(t):document.createElement(String(t));return s&&b(c,s),l(c,n),c}function y(){return new Error().stack?.split(`
`)?.[3]?.trim().replace(/^at\s+/,"")}function b(t,s){for(let n in s){if(n==="key")continue;let c=s[n];if(n==="ref"){typeof c=="function"?c(t):c&&typeof c=="object"&&(c.current=t);continue}if(n==="class"||n==="className"){v(t,c);continue}if(n==="style"){N(t,c);continue}if(n==="dataset"&&c&&typeof c=="object"){for(let o in c)t.dataset[o]=String(c[o]);continue}if(n==="dangerouslySetInnerHTML"&&c&&typeof c=="object"){t.innerHTML=c.__html??"";continue}if(n.startsWith("on")&&typeof c=="function"){let o=n.slice(2).toLowerCase();t.addEventListener(o,c);continue}let a=n==="for"?"htmlFor":n;try{a in t&&typeof t[a]!="function"?t[a]=c:c!==!1&&c!=null&&t.setAttribute(n,String(c))}catch{c!==!1&&c!=null&&t.setAttribute(n,String(c))}}}function v(t,s){s&&(typeof s=="string"?t.className=s:Array.isArray(s)?t.className=s.filter(Boolean).join(" "):typeof s=="object"&&(t.className=Object.keys(s).filter(n=>s[n]).join(" ")))}function N(t,s){if(!s)return;if(typeof s=="string"){t.style.cssText=s;return}let n=t.style;for(let c in s){let a=s[c];a==null||a===!1||(c in n?n[c]=String(a):t.style.setProperty(c,String(a)))}}function l(t,s){let n=c=>{c==null||c===!1||(Array.isArray(c)?c.forEach(n):c instanceof Node?t.appendChild(c):t.appendChild(document.createTextNode(String(c))))};s.forEach(n)}function h(t){return t.length<=1?t[0]:t}var g=document.createElement("style");g.textContent=`
  /* Remove pseudo-elements from .hip-lp */
  .hip-lp::before,
  .hip-lp::after {
    display: none !important;
